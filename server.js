...
if (process.env.GEMINI_API_KEY && process.env.GEMINI_API_KEY !== 'your_gemini_api_key_here') {
  ...
  console.warn('Warning: GEMINI_API_KEY not configured. Gemini features will be disabled.');
}
...
if (process.env.ANTHROPIC_API_KEY && process.env.ANTHROPIC_API_KEY !== 'your_anthropic_api_key_here') {
  ...
  console.warn('Warning: ANTHROPIC_API_KEY not configured. Claude API features will use CLI fallback.');
}
 ...
 * Uses the Anthropic API with Claude 3.5 Sonnet model
 * Falls back to CLI if API key not configured
 */
class ClaudeProvider extends AIProvider {
  async generateText(prompt) {
    ...
    // Fallback to CLI if API not configured
    console.log('Using Claude CLI fallback (API key not configured)');
    return new Promise((resolve, reject) => {
      ...
      claudeProcess.on('close', (code) => {
        if (code === 0) {
          ...
          reject(new Error(errorOutput || `Claude CLI process exited with code ${code}`));
        }
      });

      claudeProcess.on('error', (error) => {
        reject(new Error(`Claude CLI not found. Please set ANTHROPIC_API_KEY or install Claude Code CLI.`));
      });
    });
  }
 ...
 * @param {string} providerName - 'claude' or 'gemini'
 * @returns {AIProvider}
 */
function getAIProvider(providerName = 'claude') {
  switch (providerName.toLowerCase()) {
    ...
    default:
      console.warn(`Unknown provider "${providerName}", defaulting to Claude`);
      return new ClaudeProvider();
  }
}

/**
 * Enhance prompts specifically for Gemini 3's structured format preference
 * Adds design system, examples, and quality checks that Gemini responds better to
 * @param {string} basePrompt - The base prompt to enhance
 * @param {string} provider - The AI provider name
 * @returns {string} Enhanced prompt (unchanged for Claude, optimized for Gemini)
 */
function enhancePromptForProvider(basePrompt, provider) {
  // Only enhance for Gemini - Claude works great with current prompts
